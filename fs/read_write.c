...
{
	return file->f_mode & FMODE_STREAM ? 0 : file->f_pos;		//[yc] File is stream-like -> then start from beginning.
}
...
ssize_t ksys_read(unsigned int fd, char __user *buf, size_t count)
{
	...
	if (f.file) {
		loff_t pos = file_pos_read(f.file);			// [yhc] read saved pos.
		ret = vfs_read(f.file, buf, count, &pos);
		if (ret >= 0)
			file_pos_write(f.file, pos);			// [yhc] write updated pos.
		fdput_pos(f);
	}
	return ret;
}
